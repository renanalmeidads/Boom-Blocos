...
@export var explode_particles_scene: PackedScene = preload("res://scenes/ball/explode_particles.tscn")
...
func color_based_on_velocity() -> void:
	...
	$SpeedParticles.emitting = velocity.length() > speed + speed_threshold and not $SpeedParticles.emitting
...
func bump_boost(who) -> void:
	...
	print(frames_since_paddle_collison)
	...
	if frames_since_paddle_collison > 8 and frames_since_paddle_collison < 20:
		print("BUMP LATE")
	...
	# Bump perfect
	elif frames_since_paddle_collison < 8:
		print("BUMP PERFECT")
